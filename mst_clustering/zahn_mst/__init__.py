...
def zahn_mst(X, d=3, sigma_T=3.0, f_T=2.0, verbose=True):
    """
    ...
    if verbose:
        ...
        ie = nc - 1
        print(
            f"The algorithm found {nc} cluster(s) by removing {ie} inconsistent edge(s)."
        )
...
def _generate_full_mst(X):
    ...
    nx.set_edge_attributes(G, weights, "bw")
    ...
    for v in G.nodes:
        weights = list(
            nx.get_edge_attributes(
                G.edge_subgraph(nx.dfs_edges(G, v, d)), "bw"
            ).values()
        )
